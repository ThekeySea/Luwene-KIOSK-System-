    ...
    <div class="max-w-5xl mx-auto md:grid md:grid-cols-[40%_1fr] md:gap-8 md:items-start md:pt-6 md:pb-12 product-detail-layout">
        ...
        <div class="px-4 pt-5 pb-32 md:px-0 md:pt-0 md:pb-0">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h2 class="text-xl md:text-2xl font-display font-bold text-dark">{{ $product->name }}</h2>
                    <p class="text-sm md:text-base text-warm-500 mt-1">{{ $product->category->name }}</p>
                </div>
                <p class="text-xl md:text-2xl font-bold text-primary shrink-0">Rp {{ number_format($variantPrice, 0, ',', '.') }}</p>
            </div>

            @if ($product->description)
                <p class="text-sm md:text-base text-warm-600 mt-3">{{ $product->description }}</p>
            @endif
                ...
                <div class="space-y-6 mt-6">
                            ...
                            <div class="space-y-2">
                                @foreach ($product->variants as $variant)
                                    <label class="relative flex items-center gap-3 p-3 md:p-4 rounded-xl border-2 cursor-pointer transition
                                        ...
                                        <div class="flex-1">
                                            <span class="text-sm md:text-base {{ $selectedVariant === $variant->id ? 'font-bold text-primary' : 'font-medium text-dark' }}">{{ $variant->name }}</span>
                                        </div>
                                        <span class="text-sm md:text-base font-semibold text-primary">Rp {{ number_format($variant->price, 0, ',', '.') }}</span>
                            ...
                            <div class="space-y-2">
                                @foreach ($nasiGroup->modifiers as $nasi)
                                    <label class="relative flex items-center gap-3 p-3 md:p-4 rounded-xl border-2 cursor-pointer transition
                                        ...
                                        <div class="flex-1">
                                            <span class="text-sm md:text-base {{ $selectedNasi === $nasi->id ? 'font-bold text-primary' : 'font-medium text-dark' }}">{{ $nasi->name }}</span>
                                        </div>
                                        @if($nasi->price > 0)
                            ...
                            <div class="grid grid-cols-3 md:grid-cols-4 gap-2 md:gap-3">
                                @foreach ($sambals as $sambal)
                                    <label class="relative flex flex-col items-center p-3 md:p-4 rounded-xl border-2 cursor-pointer transition
                                        ...
                                        <span class="text-xs md:text-sm font-medium text-dark text-center">{{ $sambal->name }}</span>
                    ...
                    @if ($spiceLevels->count() > 0)
                        <div>
                            ...
                            <div class="grid grid-cols-{{ min($spiceLevels->count(), 4) }} md:grid-cols-{{ min($spiceLevels->count(), 5) }} gap-2 md:gap-3">
                                @foreach ($spiceLevels as $level)
                                    <label class="relative flex flex-col items-center p-3 md:p-4 rounded-xl border-2 cursor-pointer transition
                                        ...
                                        <span class="text-xs md:text-sm font-medium text-dark">{{ $level->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                            ...
                            <div class="space-y-2">
                                @foreach ($extraGroup->modifiers as $modifier)
                                    <label class="flex items-center gap-3 p-3 md:p-4 bg-white rounded-xl border-2 cursor-pointer transition
                                        ...
                                        <div class="flex-1">
                                            <span class="text-sm md:text-base font-medium text-dark">{{ $modifier->name }}</span>
                                        </div>
                    ...
                    <div class="hidden md:block pt-4 desktop-only">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-primary">Rp {{ number_format($totalPrice, 0, ',', '.') }}</p>
                            </div>
