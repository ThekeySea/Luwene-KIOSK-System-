<div class="min-h-dvh bg-dark-950/60 flex items-center justify-center p-4" x-data="{ show: false }" x-init="setTimeout(() => show = true, 80)">
    ...
    >
        ...
        <p class="text-sm text-warm-500 mt-1">Terima kasih, {{ $order->customer_name ?? 'pelanggan LUWENE' }}!</p>

        <div class="mt-5 bg-warm-50 rounded-2xl p-4 text-left space-y-2">
            <div class="flex justify-between text-sm">
                <span class="text-warm-500">Nomor Pesanan</span>
                <span class="font-bold text-dark">#{{ $order->order_number }}</span>
            </div>
            <div class="flex justify-between text-sm">
            ...
            <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-dark">{{ ($order->payment?->method ?? 'CASH') === 'QRIS' ? 'QRIS' : 'Tunai' }}</span>
            </div>
        </div>

        <p class="text-xs text-warm-400 mt-3">Tunjukkan nomor pesanan ini ke kasir bila diperlukan.</p>
        <p class="text-xs text-warm-400 mt-1">Struk belanja otomatis terunduh. <a href="{{ route('customer.order-receipt', $order->id) }}" class="text-primary font-medium hover:underline">Unduh ulang</a></p>

        <div class="mt-5 space-y-2.5">
            <a href="{{ route('customer.menu') }}" class="block w-full py-3.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary-700 transition">
                Kembali ke Menu
            </a>
            <a href="{{ route('home') }}" class="block w-full py-3.5 bg-white border-2 border-warm-200 text-dark font-semibold rounded-xl hover:border-primary hover:text-primary transition">
