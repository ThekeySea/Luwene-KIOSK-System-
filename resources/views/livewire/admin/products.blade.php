                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 truncate">{{ $product->slug }}@if($product->is_featured) &middot; <span class="text-primary">Favorit</span>@endif</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-gray-600">{{ $product->category->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-gray-900">Rp {{ number_format($product->base_price, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                @if($product->is_published)
                                    <button wire:click="unpublish('{{ $product->id }}')" class="px-3 py-1.5 bg-yellow-50 text-yellow-700 text-xs font-medium rounded-lg hover:bg-yellow-100 transition">Unpublish</button>
                                @else
                                    <button wire:click="$set('confirmingPublish', '{{ $product->id }}')" class="px-3 py-1.5 bg-blue-50 text-blue-700 text-xs font-medium rounded-lg hover:bg-blue-100 transition">Publish</button>
                                @endif
                                <button wire:click="openEdit('{{ $product->id }}')" class="px-3 py-1.5 bg-gray-100 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-200 transition">Edit</button>
                                <button wire:click="$set('confirmingDelete', '{{ $product->id }}')" class="px-3 py-1.5 bg-red-50 text-red-600 text-xs font-medium rounded-lg hover:bg-red-100 transition">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @empty
...
@if($showModal)
    <div class="fixed inset-0 z-40 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg bg-white rounded-xl shadow-xl p-6 max-h-[90dvh] overflow-y-auto">
            <h2 class="font-display font-bold text-gray-900 text-lg mb-4">{{ $editingId ? 'Edit Produk' : 'Tambah Produk' }}</h2>
                ...
                <div class="space-y-4">
                    ...
                    @if ($nasiModifiers->count() > 0)
                        <div>
                            ...
                            <div class="space-y-1">
                                @foreach ($nasiModifiers as $nasi)
                                    <label class="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition text-sm">
                                        <input type="checkbox" wire:model="selectedNasi" value="{{ $nasi->id }}" class="w-4 h-4 accent-primary rounded">
                                        <span class="flex-1 text-gray-700">{{ $nasi->name }}</span>
                                        <span class="text-xs text-gray-400">{{ $nasi->price > 0 ? '+Rp '.number_format($nasi->price, 0, ',', '.') : 'Gratis' }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if ($allSambals->count() > 0)
                        <div>
                            ...
                            <div class="space-y-2">
                                @foreach ($allSambals as $sambal)
                                    <div class="p-2 rounded-lg border border-gray-100 {{ in_array($sambal->id, $selectedSambals) ? 'bg-gray-50' : '' }}">
                                        ...
                                        @if (in_array($sambal->id, $selectedSambals))
                                            <div class="mt-2 ml-6 flex items-center gap-2">
                                                ...
                                                <input type="number" wire:model.live="sambalPrices.{{ $sambal->id }}" min="0" class="w-28 px-2 py-1 bg-white border border-gray-200 rounded text-xs text-gray-900 focus:ring-1 focus:ring-primary outline-none" placeholder="0" />
                    ...
                    @if ($extraModifiers->count() > 0)
                        <div>
                            ...
                            <div class="space-y-1">
                                @foreach ($extraModifiers as $extra)
                                    <label class="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition text-sm">
                                        <input type="checkbox" wire:model="selectedExtras" value="{{ $extra->id }}" class="w-4 h-4 accent-accent rounded">
                                        <span class="flex-1 text-gray-700">{{ $extra->name }}</span>
                                        <span class="text-xs text-gray-400">{{ $extra->price > 0 ? '+Rp '.number_format($extra->price, 0, ',', '.') : 'Gratis' }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
