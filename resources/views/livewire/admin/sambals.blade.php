    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse ($sambals as $sambal)
                        <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition">
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $sambal->name }}</div>
                                @if ($sambal->description)
                                    <div class="text-xs text-gray-400 mt-0.5">{{ Str::limit($sambal->description, 50) }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-700">
                                {{ $sambal->price > 0 ? 'Rp ' . number_format($sambal->price, 0, ',', '.') : 'Gratis' }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700">
                                    {{ $sambal->spice_levels_count }} level
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center text-gray-600">{{ $sambal->products_count }}</td>
                            <td class="px-4 py-3 text-center">
                                @if ($sambal->is_active)
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="toggleActive('{{ $sambal->id }}')" class="px-2 py-1 text-xs rounded-lg {{ $sambal->is_active ? 'text-gray-500 hover:bg-gray-100' : 'text-green-600 hover:bg-green-50' }} transition" title="{{ $sambal->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        {{ $sambal->is_active ? 'Nonaktif' : 'Aktif' }}
                                    </button>
                                    <button wire:click="openEdit('{{ $sambal->id }}')" class="px-2 py-1 text-xs text-blue-600 rounded-lg hover:bg-blue-50 transition">Edit</button>
                                    <button wire:click="$set('confirmingDelete', '{{ $sambal->id }}')" class="px-2 py-1 text-xs text-red-600 rounded-lg hover:bg-red-50 transition">Hapus</button>
                                </div>
                            </td>
                        </tr>
                    @empty
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data>
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <div class="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 rounded-t-2xl">
                    <h3 class="text-lg font-bold text-gray-900">{{ $editingId ? 'Edit Sambal' : 'Tambah Sambal' }}</h3>
                </div>
                ...
                <form wire:submit.prevent="save" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        @if (count($spiceLevels) > 0)
                            <div class="space-y-2 mb-3">
                                @foreach ($spiceLevels as $i => $level)
                                    <div class="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                                        <span class="w-6 h-6 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-xs font-bold shrink-0">{{ $i }}</span>
                                        <span class="flex-1 text-sm text-gray-700">{{ $level['name'] }}</span>
                                        <button type="button" wire:click="removeLevel({{ $i }})" class="text-red-400 hover:text-red-600 transition text-xs">Hapus</button>
                                    </div>
                                @endforeach
                            </div>
                        @endif
