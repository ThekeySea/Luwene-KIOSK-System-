<div class="min-h-dvh flex flex-col bg-warm-50">
    <header class="bg-white shadow-sm shrink-0">
        <div class="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
            <a href="{{ route('delivery.entry') }}" class="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-warm-100 text-warm-500 hover:text-dark transition">
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-8">
        <div class="w-full max-w-sm">
            ...
            <p class="text-center text-sm text-warm-500 mt-6">
                Belum punya akun?
                <a href="{{ route('delivery.register') }}" class="text-primary font-semibold hover:underline">Daftar</a>
            </p>
        </div>
    </main>
</div>
