...
<body class="bg-warm-50 text-dark antialiased min-h-dvh font-sans">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showNav ?? true): ?>
        <?php
        ...
        <nav class="fixed bottom-0 left-0 right-0 z-30 px-4" style="padding-bottom: calc(env(safe-area-inset-bottom) + 0.75rem);">
            <div class="max-w-lg mx-auto bg-white rounded-full border border-warm-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] px-2 py-2 grid grid-cols-5 items-center">
                <a href="<?php echo e(route('home')); ?>" class="flex flex-col items-center gap-0.5 py-1 transition <?php echo e($isHome ? 'text-primary' : 'text-warm-400 hover:text-warm-600'); ?>">
                    ...
                    <span class="text-xs <?php echo e($isHome ? 'font-bold' : 'font-medium'); ?>">Beranda</span>
                </a>
                <a href="<?php echo e(route('customer.menu')); ?>" class="flex flex-col items-center gap-0.5 py-1 transition <?php echo e($isMenu ? 'text-primary' : 'text-warm-400 hover:text-warm-600'); ?>">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <a href="<?php echo e(route('customer.cart')); ?>" class="flex flex-col items-center" aria-label="Nampan">
                    <span class="relative -mt-8 w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg ring-4 ring-warm-50 transition <?php echo e($isCart ? 'bg-accent' : 'bg-primary hover:bg-primary-700'); ?>">
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($navCartCount > 0): ?>
                            <span class="absolute -top-1 -right-1 min-w-6 h-6 px-1.5 bg-accent text-white text-xs font-bold rounded-full flex items-center justify-center ring-2 ring-white <?php echo e($isCart ? 'bg-dark' : ''); ?>"><?php echo e($navCartCount > 99 ? '99+' : $navCartCount); ?></span>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </span>
                    <span class="text-xs mt-1 <?php echo e($isCart ? 'font-bold text-primary' : 'font-medium text-warm-500'); ?>">Nampan</span>
                </a>
                <a href="<?php echo e(route('customer.promo')); ?>" class="flex flex-col items-center gap-0.5 py-1 transition <?php echo e($isPromo ? 'text-primary' : 'text-warm-400 hover:text-warm-600'); ?>">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span class="text-xs <?php echo e($isPromo ? 'font-bold' : 'font-medium'); ?>">Promo</span>
                </a>
                <a href="<?php echo e(route('customer.faq')); ?>" class="flex flex-col items-center gap-0.5 py-1 transition <?php echo e($isFaq ? 'text-primary' : 'text-warm-400 hover:text-warm-600'); ?>">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<?php /**PATH C:\laragon\www\Luwene\resources\views/components/layouts/customer.blade.php ENDPATH**/ ?>